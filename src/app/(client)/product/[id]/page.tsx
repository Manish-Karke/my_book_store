    ...
    <>
      ...
      <section className="custom-height relative bg-[#f5f5f5]">
        <div className="z-50 mx-auto flex h-full max-w-6xl gap-x-10 px-5 py-14 md:py-20">
          <div>
            {isLoading ? (
              <Skeleton className="aspect-square w-[28rem] rounded-md  bg-brown-100 object-cover shadw-2xl" />
            ) : (
              <Image
                src={
                  product?.image ? `/assets/${product.image}` : "/product1.jpg"
                }
                alt={product?.name ?? "image"}
                width={500}
                height={500}
                sizes="100vw"
                className="aspect-square w-[28rem] rounded-md object-cover shadw-2xl"
              />
            )}
          </div>
